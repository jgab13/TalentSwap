...
class CourseFilter extends React.Component{
    ...
    handleLevelFilter(ek){
        console.log(typeof(this.state.filters))
        const newFilters = this.state.filters.concat([`level:${ek}`]) // SearchPage updateState will parse it based on this particular format
        this.setState({
            filters: newFilters
        })
